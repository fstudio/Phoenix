...
void CommandLineArguments::GetRemainingArguments(int* argc, wchar_t*** argv)
{
  ...
  wcscpy(args[0], this->Internals->Argv0.c_str());
  int cnt = 1;
  ...
  for ( cc = this->Internals->LastArgument+1; 
    ...
    wcscpy(args[cnt], this->Internals->Argv[cc].c_str());
    cnt ++;
    }
  *argc = cnt;
  *argv = args;
...
void CommandLineArguments::GenerateHelp()
{
  ...
  wchar_t format[80];
  swprintf(format, L"  %%-%us  ", static_cast<unsigned int>(maxlen));
  ...
  for ( mpit = mp.begin();
    ...
    for ( sit = mpit->second.begin(); sit != mpit->second.end(); sit++ )
      ...
      swprintf(argument, L"%s", sit->c_str());
      switch ( this->Internals->Callbacks[*sit].ArgumentType )
        {
      ...
      wchar_t buffer[80];
      swprintf(buffer, format, argument);
      str << buffer;
      }
