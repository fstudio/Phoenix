...
/*--------------------------------------------------------------------------*/
static const unsigned char KwBase64EncodeTable[65] =
"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
"abcdefghijklmnopqrstuvwxyz"
"0123456789+/";

/*--------------------------------------------------------------------------*/
static const unsigned char KwBase64DecodeTable[256] =
{
...
/*--------------------------------------------------------------------------*/
static unsigned char KwBase64EncodeChar(int c)
{
  return KwBase64EncodeTable[(unsigned char)c];
}

/*--------------------------------------------------------------------------*/
static unsigned char KwBase64DecodeChar(unsigned char c)
{
  return KwBase64DecodeTable[c];
}

/*--------------------------------------------------------------------------*/
/* Encode 3 bytes into a 4 byte string. */
void KwBase64_Encode3(const unsigned char *src, unsigned char *dest)
{
  dest[0] = KwBase64EncodeChar((src[0] >> 2) & 0x3F);
  dest[1] = KwBase64EncodeChar(((src[0] << 4) & 0x30)|((src[1] >> 4) & 0x0F));
  dest[2] = KwBase64EncodeChar(((src[1] << 2) & 0x3C)|((src[2] >> 6) & 0x03));
  dest[3] = KwBase64EncodeChar(src[2] & 0x3F);
}

/*--------------------------------------------------------------------------*/
/* Encode 2 bytes into a 4 byte string. */
void KwBase64_Encode2(const unsigned char *src, unsigned char *dest)
{
  dest[0] = KwBase64EncodeChar((src[0] >> 2) & 0x3F);
  dest[1] = KwBase64EncodeChar(((src[0] << 4) & 0x30)|((src[1] >> 4) & 0x0F));
  dest[2] = KwBase64EncodeChar(((src[1] << 2) & 0x3C));
  dest[3] = '=';
}

/*--------------------------------------------------------------------------*/
/* Encode 1 bytes into a 4 byte string. */
void KwBase64_Encode1(const unsigned char *src, unsigned char *dest)
{
  dest[0] = KwBase64EncodeChar((src[0] >> 2) & 0x3F);
  dest[1] = KwBase64EncodeChar(((src[0] << 4) & 0x30));
  dest[2] = '=';
  dest[3] = '=';
}
...
int KwBase64_Decode3(const unsigned char *src, unsigned char *dest)
{
  unsigned char d0, d1, d2, d3;

  d0 = KwBase64DecodeChar(src[0]);
  d1 = KwBase64DecodeChar(src[1]);
  d2 = KwBase64DecodeChar(src[2]);
  d3 = KwBase64DecodeChar(src[3]);
                          ...
                          size_t max_input_length)
{
  ...
  if (max_input_length)
    ...
    while (ptr < end)
      {
      int len = KwBase64_Decode3(ptr, optr);
      optr += len;
      if(len < 3)
        {
        return (size_t)(optr - output);
    ...
    if (oend - optr == 2)
      ...
      int len = KwBase64_Decode3(ptr, temp);
      if(len >= 2)
        {
        optr[0] = temp[0];
        optr[1] = temp[1];
