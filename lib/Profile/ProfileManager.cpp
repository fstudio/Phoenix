...
private:
  ...
  bool XmlAttributesResolve(IXmlReader* pReader, std::map<std::wstring, std::wstring> &kv)
  {
          ...
          else
          {
            ...
            else
            {
              if (wcscmp(pwszLocalName, L"value") == 0)
              {
                ...
                kv.insert(std::map<std::wstring, std::wstring>::value_type(pwszKey, pwszValue));
  ...
  HRESULT XmlReaderInternal(std::map<std::wstring, std::wstring> &kv)
  {
    ...
    HRESULT hr = S_OK;
    if ((hr = SHCreateStreamOnFile(m_xmlfile.c_str(), STGM_READ, &pXmlStream)) != S_OK)
      return hr;
    if ((hr = CreateXmlReader(__uuidof(IXmlReader), (void**) &pReader, NULL)) != S_OK)
      return hr;
    if ((hr = pReader->SetProperty(XmlReaderProperty_DtdProcessing, DtdProcessing_Prohibit)) != S_OK)
      return hr;
...
public:
  XmlIntegratedAnalyzer(std::wstring xmlfile) :m_xmlfile(xmlfile) ,pXmlStream(nullptr),pReader(nullptr)
  {
  }
