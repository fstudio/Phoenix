...
#define CRTMalloc malloc
#define CRTFree free
#define CRTRealloc realloc
...
node_alloc(void *p)
{
	...
	if (E.free==-1) {
		...
		E.pool=(struct node *)CRTRealloc(E.pool,sz*sizeof(struct node));
		ret=E.pool + E.size;
		ret->u.n.children=0;
...
link_expand(struct link *old,int sz)
{
	...
	ret=(struct link *)CRTRealloc(old,sizeof(struct link)+(sz-1)*sizeof(int));
	if (old==0) {
		ret->number=0;
	}
	return ret;
...
map_expand()
{
	...
	table=(struct hash_node	**)CRTMalloc(sz*sizeof(struct hash_node *));
...
map_id(void *p)
{
	...
	if (E.map.free) {
		...
		node=(struct hash_node *)CRTMalloc(sizeof(*node));
	}
...
stack_expand()
{
	if (((E.stack.top + 1) ^ E.stack.top) > E.stack.top) {
		E.stack.data = (union stack_node *)CRTRealloc(E.stack.data,(E.stack.top*2+1) * sizeof(union stack_node));
	}
}
...
{
	struct gc_weak_table *ret=CRTMalloc(sizeof(*ret));
